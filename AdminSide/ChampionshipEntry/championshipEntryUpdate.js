                ...
                response.forEach((e) => {
                    $('#rows').append(`
                        <tr>
                            ...
                            <td><input id="points" class="form-control" type="text" value="${e.championshipEntryTotalPoints}" aria-label="Disabled input example" disabled ></td>
...
$(document).on('click','.btn-dark', (event) => {
    $.ajax({
        'url': 'http://localhost/gt3prostats/backend/api/championshipentry/updatechampionshipEntry.php',
        'data': {
            'championshipEntryID' : $(event.target).val(),
            'championshipEntryTotalPoints' : $('#points').val()
        },
        'type': 'post',
        'dataType': 'html',
        'beforeSend':  () => {
